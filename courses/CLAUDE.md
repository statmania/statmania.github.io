...
- `courses/index.html` — the course **hub page**, hand-written static HTML
  (Tailwind CDN + `css/dark-theme.css`, matching `games/`/`utility/`). One
  card per course, links to `<slug>/index.html`. Edit this directly when
...
## Why Quarto here (and not hand-written HTML like games/utility)
...
multi-lesson sequence), hand-written HTML matching `games/`/`utility/` is
still the better fit — don't force everything into Quarto.
...
5. `quarto render`.
6. Add a card for it in `courses/index.html`.
