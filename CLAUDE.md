...
Every **other** page under `ds/`, `games/`, `utility/` (e.g. `games/sudoku.html`, `utility/bmi.html`) is **not** Quarto-rendered — hand-written static HTML (self-contained with inline `<style>`/`<script>`, no shared build step, still using the per-directory `css/dark-theme.css` + the shared root `starfield.js`). Edit those `.html` files directly.
...
There is **one** starfield script: `/starfield.js` in the repo root (not generated by any Quarto project). Every section loads it with a relative `<script src="../starfield.js">` (or `starfield.js` from the root) — never copy it into a section. It draws into `canvas.sm-canvas`, `#sm-canvas` (raw/ pages) and `#sm-slide-canvas` (slides). Quarto projects (blog, courses) add it with the shared include `starfield-include.html` in the repo root (`include-after-body: ../starfield-include.html`, path relative to the project), which adds the canvas and resolves the script's root path from the URL. Slides load it from `slide/_quarto.yml`.
