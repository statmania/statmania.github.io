...
<style>
  ...
  .cell { width: 40px; height: 40px; background: rgba(255,255,255,0.06); position: relative; border-radius: 2px; }
  ...
  .cell .num { position: absolute; top: 1px; left: 3px; font-size: 0.6rem;
               color: var(--muted); font-weight: 600; pointer-events: none; }
  .cell.in-word { background: var(--active-word); }
  .cell.active {
  ...
  .cell.active input { color: var(--accent); }
  .cell.correct input { color: #4ade80; }
  .cell.wrong input { color: #ff6b6b; }
...
function updateHighlights() {
  document.querySelectorAll(".cell").forEach(el => el.classList.remove("active", "in-word"));
  ...
  const activeEl = document.querySelector(
    `.cell[data-r="${state.current.r}"][data-c="${state.current.c}"]`);
